...
public final class RedisUtil {
  ...
  @Resource public RedisTemplate<String, Object> redisTemplate;
  ...
  /**
   * setIfAbsent() 方法 返回值为布尔类型 方法类似redis的SETNX命令 即”SET if Not Exists”
   *
   * @param key
   * @param value
   * @return
  ...
  public boolean tryLock(String key, Object value, long timeout) {
    ...
    // 底层原理就是Redis的setnx方法
    boolean isSuccess = redisTemplate.opsForValue().setIfAbsent(key, String.valueOf(value));
    if (isSuccess) {
      // 设置分布式锁的过期时间
